...
export class ChatService {
  ...
  async getSessions() {
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error('Get sessions error:', errorText);
  ...
  async sendMessage(
    ...
    try {
      ...
      // Clean conversation history for backend
      const cleanHistory = conversationHistory.map((msg) => {
        let content = msg.content;

        // Convert structured response objects to strings
        if (
      ...
      if (typeof data.response === 'string') {
        try {
          ...
          // Response is normal text, not JSON
        }
      }
